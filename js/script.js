...
/* =========================================================
   MOTION UI V15 — ligero, accesible y sin dependencias
========================================================= */
(() => {
  ...
  const preview = document.querySelector('.motion-tilt');
  ...
  if (preview && canTilt) {
    preview.addEventListener('pointermove', (event) => {
      ...
      preview.style.setProperty('--rotate-x', `${y * -4}deg`);
      preview.style.setProperty('--rotate-y', `${x * 5}deg`);
    });
    preview.addEventListener('pointerleave', () => {
      preview.style.setProperty('--rotate-x', '0deg');
      preview.style.setProperty('--rotate-y', '-3deg');
    });
  }
...
/* GAP v17: progress and interactive card states */
(() => {
  const form = document.querySelector('#gapFormModal');
  const grid = document.querySelector('#gapGridModal');
  if (!form || !grid) return;

  const total = () => grid.querySelectorAll('select').length;
  const labelText = 'Progreso';
  const progress = document.createElement('div');
  progress.className = 'gapProgress';
  progress.innerHTML = `
    <span class="gapProgress__label">${labelText}</span>
    <span class="gapProgress__track" aria-hidden="true"><i class="gapProgress__bar"></i></span>
    <span class="gapProgress__value" aria-live="polite"></span>`;
  form.insertBefore(progress, grid);

  const value = progress.querySelector('.gapProgress__value');
  const update = () => {
    const selects = [...grid.querySelectorAll('select')];
    const completed = selects.filter((select) => select.value !== '').length;
    const percent = selects.length ? Math.round((completed / selects.length) * 100) : 0;
    progress.style.setProperty('--gap-progress', `${percent}%`);
    value.textContent = `${completed} de ${selects.length}`;
    selects.forEach((select) => select.closest('.gapQ')?.classList.toggle('is-complete', select.value !== ''));
  };
